...
import { Button, Col, Divider, Form, InputNumber, message, Modal, Row, Select, Space } from 'antd';
...
export const ExportBtn: React.FC<IExportBtn> = ({ model, search, sortBy, filters, style }) => {
  ...
  const { mutate: mutateExport, isLoading: isLoadingExport } = useMutation(
    (payload: any) => postFetcher(`/export/${model}?${exportQueryString}`, payload),
    {
      onSuccess: (data) => {
        const fileData =
          form.getFieldValue('format') === EExportFormat.JSON ? JSON.stringify(data) : data;
        fileDownload(fileData, `${model}.${form.getFieldValue('format').toLowerCase()}`);
        setOpen(false);
        form.resetFields();
  ...
  const onExport = useCallback((data: any) => mutateExport(data), [mutateExport]);
  ...
  return (
    <>
      ...
      <Modal width={300} open={open} onCancel={onClose} footer={null} title={_t('Export')}>
        <Divider />
        <Row>
          <Col xs={24}>
            <Form
